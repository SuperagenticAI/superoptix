...
class MilvusVectorStore(VectorStoreInterface):
    ...
    def __init__(self, client, collection_name: str, embedding_function=None):
        ...
        if not self.client.has_collection(collection_name):
            raise ValueError(
                f"Collection '{collection_name}' not found. Please create the collection first."
            )
    ...
    def add_documents(
        ...
        try:
            ...
            if "insert_count" in result and result["insert_count"] == len(
                data_to_insert
            ):
                return ids
            else:
    ...
    def _format_results(self, results) -> list[dict[str, Any]]:
        ...
        for hit in hits:
            ...
            score = (
                max(0.0, 1.0 - distance) if distance <= 1.0 else 1.0 / (1.0 + distance)
            )
