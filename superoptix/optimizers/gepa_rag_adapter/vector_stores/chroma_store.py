...
class ChromaVectorStore(VectorStoreInterface):
    ...
    def _format_results(self, results) -> list[dict[str, Any]]:
        ...
        for doc, metadata, distance in zip(docs, metadatas, distances, strict=False):
            ...
            documents.append(
                {"content": doc, "metadata": metadata or {}, "score": similarity_score}
            )
    ...
    def _extract_distance_value(self, distance) -> float:
        ...
        try:
            if hasattr(distance, "item"):  # numpy scalar
                ...
                hasattr(distance, "__len__")
                and not isinstance(distance, (str, bytes))
                and len(distance) == 1
            ):
            ...
            logging.warning(
                f"Unexpected distance format: {type(distance)}, value: {distance}, error: {e}"
            )
            return 0.0  # Default fallback

    @classmethod
    def create_local(
        cls, persist_directory: str, collection_name: str, embedding_function=None
    ) -> "ChromaVectorStore":
        """
    ...
    @classmethod
    def create_memory(
        cls, collection_name: str, embedding_function=None
    ) -> "ChromaVectorStore":
        """
