...
class WeaviateVectorStore(VectorStoreInterface):
    ...
    def __init__(self, client, collection_name: str, embedding_function=None):
        ...
        if importlib.util.find_spec("weaviate") is None:
            raise ImportError(
                "Weaviate client is required for WeaviateVectorStore. Install with: uv pip install litellm weaviate-client"
            )
    ...
    def hybrid_search(
        ...
        try:
            results = self.collection.query.hybrid(
                ...
                limit=k,
                where=weaviate_filters,
                return_metadata=self.wvc.query.MetadataQuery(
                ...
                if hasattr(config.vector_config, "vector_index_config"):
                    ...
                    if hasattr(vector_index, "distance"):
                        ...
                        try:
                            ...
                            if (
                                sample.objects
                                and hasattr(sample.objects[0], "vector")
                                and sample.objects[0].vector
                            ):
                ...
                for op, op_value in value.items():
                    if op == "$eq" or op == "equal":
                        filter_conditions.append(
                            Filter.by_property(key).equal(op_value)
                        )
                    elif op == "$ne" or op == "not_equal":
                        filter_conditions.append(
                            Filter.by_property(key).not_equal(op_value)
                        )
                    elif op == "$gt" or op == "greater_than":
                        filter_conditions.append(
                            Filter.by_property(key).greater_than(op_value)
                        )
                    elif op == "$gte" or op == "greater_equal":
                        filter_conditions.append(
                            Filter.by_property(key).greater_or_equal(op_value)
                        )
                    elif op == "$lt" or op == "less_than":
                        filter_conditions.append(
                            Filter.by_property(key).less_than(op_value)
                        )
                    elif op == "$lte" or op == "less_equal":
                        filter_conditions.append(
                            Filter.by_property(key).less_or_equal(op_value)
                        )
                    elif op == "$in" or op == "contains_any":
                        if isinstance(op_value, list):
                            filter_conditions.append(
                                Filter.by_property(key).contains_any(op_value)
                            )
    ...
    def create_local(
        ...
        try:
            ...
            raise ImportError(
                "Weaviate client is required. Install with: uv pip install litellm weaviate-client"
            ) from e

        client = weaviate.connect_to_local(
            host=host, port=port, grpc_port=grpc_port, headers=headers
        )
