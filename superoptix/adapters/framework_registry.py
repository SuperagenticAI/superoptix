...
def _write_compiled_spec_sidecar(output_path: str, spec: Dict[str, Any]) -> str:
    """
    Write resolved SuperSpec payload next to generated pipeline.

    Returns the sidecar filename (basename), intended for template context.
    """
    output_file = Path(output_path)
    sidecar = output_file.with_name(f"{output_file.stem}_compiled_spec.json")
...
class MicrosoftFrameworkAdapter(FrameworkAdapter):
    ...
    def compile_from_playbook(
        ...
        if not agent_name or agent_name == "pipeline":
            ...
            # Ensure agent_name is a valid identifier fragment.
            agent_name = to_snake_case(agent_name)
    ...
    def create_component(cls, playbook: Dict[str, Any], **kwargs) -> BaseComponent:
        ...
        # Universal GEPA optimization needs the full component wrapper (BaseComponent),
        # which is generated by the optimized profile template.
        cls.compile_from_playbook(
            playbook,
            str(output_path),
            compile_profile="optimized",
        )
        ...
        # Prefer exact generated name, but fall back to first *Component class
        # to avoid metadata/display-name mismatches.
        expected_component_name = f"{agent_name_pascal}Component"
        component_class = getattr(module, expected_component_name, None)
        if component_class is None:
            for name, obj in module.__dict__.items():
                if (
                    isinstance(obj, type)
                    and name.endswith("Component")
        ...
        if component_class is None:
            raise AttributeError(
                f"module '{module.__name__}' has no BaseComponent subclass "
                f"(expected '{expected_component_name}')"
            )
...
class OpenAIFrameworkAdapter(FrameworkAdapter):
    ...
    def compile_from_playbook(
        ...
        # Load template based on compile profile.
        if compile_profile == "optimized":
            template_name = "openai_pipeline_optimized.py.jinja2"
        else:
            template_name = "openai_pipeline_minimal.py.jinja2"
...
class DeepAgentsFrameworkAdapter(FrameworkAdapter):
    ...
    def compile_from_playbook(
        ...
        # Load template based on compile profile.
        if compile_profile == "optimized":
            template_name = "deepagents_pipeline_optimized.py.jinja2"
        else:
            template_name = "deepagents_pipeline_minimal.py.jinja2"
        template = env.get_template(template_name)
...
class CrewAIFrameworkAdapter(FrameworkAdapter):
    ...
    def compile_from_playbook(
        ...
        # Load template based on compile profile.
        profile = str(compile_profile or "minimal").strip().lower()
        template_name = (
            "crewai_pipeline_minimal.py.jinja2"
            if profile == "minimal"
            else "crewai_pipeline_optimized.py.jinja2"
        )
        template = env.get_template(template_name)
        ...
        if not agent_name or agent_name == "pipeline":
            agent_name = playbook.get("metadata", {}).get("name", "agent")
        # Ensure valid identifier-safe naming for class/function generation.
        agent_name = to_snake_case(agent_name)
    ...
    def create_component(cls, playbook: Dict[str, Any], **kwargs) -> BaseComponent:
        ...
        # Universal GEPA optimization requires BaseComponent wrapper from optimized profile.
        cls.compile_from_playbook(
            playbook,
            str(output_path),
            compile_profile="optimized",
...
class GoogleADKFrameworkAdapter(FrameworkAdapter):
    ...
    def compile_from_playbook(
        ...
        template_name = (
            ...
            else "google_adk_pipeline.py.jinja2"
        )
        template = env.get_template(template_name)
    ...
    def create_component(cls, playbook: Dict[str, Any], **kwargs) -> BaseComponent:
        ...
        # Universal GEPA optimization needs BaseComponent wrapper from optimized profile.
        cls.compile_from_playbook(
            playbook,
            str(output_path),
            compile_profile="optimized",
...
class PydanticAIFrameworkAdapter(FrameworkAdapter):
    ...
    def compile_from_playbook(
        ...
        template_name = (
            ...
            if compile_profile == "minimal"
            else "pydantic_ai_pipeline.py.jinja2"
        )
        template = env.get_template(template_name)
    ...
    def create_component(cls, playbook: Dict[str, Any], **kwargs) -> BaseComponent:
        ...
        # Allow runtime overrides from caller (e.g., CLI --provider/--model on optimize/run).
        model_config_override = kwargs.pop("model_config", None)
        if isinstance(model_config_override, dict) and model_config_override:
            for key, value in model_config_override.items():
                if key == "gateway" and isinstance(value, dict):
                    gw = model_config.get("gateway", {})
                    if not isinstance(gw, dict):
                        gw = {}
                    gw.update(value)
                    model_config["gateway"] = gw
                else:
...
class FrameworkRegistry:
    ...
    def compile_agent(
        ...
        """
        adapter = cls.get_adapter(framework)
        return adapter.compile_from_playbook(
            playbook, output_path, compile_profile=compile_profile
        )
